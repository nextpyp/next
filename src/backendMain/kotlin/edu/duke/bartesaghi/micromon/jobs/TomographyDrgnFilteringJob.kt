...
class TomographyDrgnFilteringJob(
	userId: String,
	projectId: String
	...
	companion object : JobInfo {
		...
		override val dataClass = TomographyDrgnFilteringData::class

		override fun fromDoc(doc: Document) = TomographyDrgnFilteringJob(
			doc.getString("userId"),
			doc.getString("projectId")
		).apply {
			args.finished = doc.getDocument("finishedArgs")?.let { TomographyDrgnFilteringArgs.fromDoc(it) }
			args.next = doc.getDocument("nextArgs")?.let { TomographyDrgnFilteringArgs.fromDoc(it) }
			latestReconstructionId = doc.getString("latestReconstructionId")
			jobInfoString = doc.getString("jobInfoString")
			fromDoc(doc)
		}

		private fun TomographyDrgnFilteringArgs.toDoc() = Document().also { doc ->
			doc["values"] = values
		}

		private fun TomographyDrgnFilteringArgs.Companion.fromDoc(doc: Document) =
			TomographyDrgnFilteringArgs(
