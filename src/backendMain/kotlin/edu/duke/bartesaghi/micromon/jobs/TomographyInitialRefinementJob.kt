...
class TomographyInitialRefinementJob(
	userId: String,
	projectId: String
	...
	companion object : JobInfo {
		...
		override val dataClass = TomographyInitialRefinementData::class

		override fun fromDoc(doc: Document) = TomographyInitialRefinementJob(
			doc.getString("userId"),
			doc.getString("projectId")
		).apply {
			args.finished = doc.getDocument("finishedArgs")?.let { TomographyInitialRefinementArgs.fromDoc(it) }
			args.next = doc.getDocument("nextArgs")?.let { TomographyInitialRefinementArgs.fromDoc(it) }
			latestReconstructionId = doc.getString("latestReconstructionId")
			jobInfoString = doc.getString("jobInfoString")
			fromDoc(doc)
		}

		private fun TomographyInitialRefinementArgs.toDoc() = Document().also { doc ->
			doc["values"] = values
			doc["filter"] = filter
		}

		private fun TomographyInitialRefinementArgs.Companion.fromDoc(doc: Document) =
			TomographyInitialRefinementArgs(
	...
	override suspend fun data() =
		TomographyInitialRefinementData(
			commonData(),
			args,
			diagramImageURL(),
			jobInfoString ?: ""
