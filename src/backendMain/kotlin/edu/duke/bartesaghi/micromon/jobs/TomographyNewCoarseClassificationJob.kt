...
class TomographyNewCoarseClassificationJob(
	userId: String,
	projectId: String
) : Job(userId, projectId, config) {

	val args = JobArgs<TomographyNewCoarseClassificationArgs>()
	var inMovieRefinement: CommonJobData.DataId? by InputProp(config.movieRefinementsIn)
	...
	companion object : JobInfo {
		...
		override val dataClass = TomographyNewCoarseClassificationData::class

		override fun fromDoc(doc: Document) = TomographyNewCoarseClassificationJob(
			doc.getString("userId"),
			doc.getString("projectId")
		).apply {
			args.finished = doc.getDocument("finishedArgs")?.let { TomographyNewCoarseClassificationArgs.fromDoc(it) }
			args.next = doc.getDocument("nextArgs")?.let { TomographyNewCoarseClassificationArgs.fromDoc(it) }
			latestReconstructionId = doc.getString("latestReconstructionId")
			jobInfoString = doc.getString("jobInfoString")
			fromDoc(doc)
		}

		private fun TomographyNewCoarseClassificationArgs.toDoc() = Document().also { doc ->
			doc["values"] = values
			doc["filter"] = filter
		}

		private fun TomographyNewCoarseClassificationArgs.Companion.fromDoc(doc: Document) =
			TomographyNewCoarseClassificationArgs(
	...
	override suspend fun data() =
		TomographyNewCoarseClassificationData(
			commonData(),
			args,
			diagramImageURL(),
			jobInfoString ?: ""
