...
class TomographyImportDataPureJob(
	userId: String,
	projectId: String
	...
	companion object : JobInfo {
		...
		override val dataClass = TomographyImportDataPureData::class

		override fun fromDoc(doc: Document) = TomographyImportDataPureJob(
			doc.getString("userId"),
			doc.getString("projectId")
		).apply {
			fromDoc(doc)
			args.finished = doc.getDocument("finishedArgs")?.let { TomographyImportDataPureArgs.fromDoc(it) }
			args.next = doc.getDocument("nextArgs")?.let { TomographyImportDataPureArgs.fromDoc(it) }
			latestTiltSeriesId = doc.getString("latestTiltSeriesId")
		}

		private fun TomographyImportDataPureArgs.toDoc() = Document().also { doc ->
			doc["values"] = values
		}

		private fun TomographyImportDataPureArgs.Companion.fromDoc(doc: Document) =
			TomographyImportDataPureArgs(
