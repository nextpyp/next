...
class TomographyAfterFlexibleRefinementJob(
	userId: String,
	projectId: String
) : Job(userId, projectId, config) {

	val args = JobArgs<TomographyAfterFlexibleRefinementArgs>()
	var inMovieAfterRefinements: CommonJobData.DataId? by InputProp(config.movieAfterRefinements)
	...
	companion object : JobInfo {
		...
		override val dataClass = TomographyAfterFlexibleRefinementData::class

		override fun fromDoc(doc: Document) = TomographyAfterFlexibleRefinementJob(
			doc.getString("userId"),
			doc.getString("projectId")
		).apply {
			args.finished = doc.getDocument("finishedArgs")?.let { TomographyAfterFlexibleRefinementArgs.fromDoc(it) }
			args.next = doc.getDocument("nextArgs")?.let { TomographyAfterFlexibleRefinementArgs.fromDoc(it) }
			latestReconstructionId = doc.getString("latestReconstructionId")
			jobInfoString = doc.getString("jobInfoString")
			fromDoc(doc)
		}

		private fun TomographyAfterFlexibleRefinementArgs.toDoc() = Document().also { doc ->
			doc["values"] = values
		}

		private fun TomographyAfterFlexibleRefinementArgs.Companion.fromDoc(doc: Document) =
			TomographyAfterFlexibleRefinementArgs(
	...
	override suspend fun data() =
		TomographyAfterFlexibleRefinementData(
			commonData(),
			args,
			diagramImageURL(),
			jobInfoString ?: ""
