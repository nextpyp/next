...
class TomographyReferenceFreeRefinementJob(
	userId: String,
	projectId: String
	...
	companion object : JobInfo {
		...
		override val dataClass = TomographyReferenceFreeRefinementData::class

		override fun fromDoc(doc: Document) = TomographyReferenceFreeRefinementJob(
			doc.getString("userId"),
			doc.getString("projectId")
		).apply {
			args.finished = doc.getDocument("finishedArgs")?.let { TomographyReferenceFreeRefinementArgs.fromDoc(it) }
			args.next = doc.getDocument("nextArgs")?.let { TomographyReferenceFreeRefinementArgs.fromDoc(it) }
			latestReconstructionId = doc.getString("latestReconstructionId")
			jobInfoString = doc.getString("jobInfoString")
			fromDoc(doc)
		}

		private fun TomographyReferenceFreeRefinementArgs.toDoc() = Document().also { doc ->
			doc["values"] = values
			doc["filter"] = filter
		}

		private fun TomographyReferenceFreeRefinementArgs.Companion.fromDoc(doc: Document) =
			TomographyReferenceFreeRefinementArgs(
	...
	override suspend fun data() =
		TomographyReferenceFreeRefinementData(
			commonData(),
			args,
			diagramImageURL(),
			jobInfoString ?: ""
