...
class TomographyReferenceBasedRefinementJob(
	userId: String,
	projectId: String
	...
	companion object : JobInfo {
		...
		override val dataClass = TomographyReferenceBasedRefinementData::class

		override fun fromDoc(doc: Document) = TomographyReferenceBasedRefinementJob(
			doc.getString("userId"),
			doc.getString("projectId")
		).apply {
			args.finished = doc.getDocument("finishedArgs")?.let { TomographyReferenceBasedRefinementArgs.fromDoc(it) }
			args.next = doc.getDocument("nextArgs")?.let { TomographyReferenceBasedRefinementArgs.fromDoc(it) }
			latestReconstructionId = doc.getString("latestReconstructionId")
			jobInfoString = doc.getString("jobInfoString")
			fromDoc(doc)
		}

		private fun TomographyReferenceBasedRefinementArgs.toDoc() = Document().also { doc ->
			doc["values"] = values
			doc["filter"] = filter
		}

		private fun TomographyReferenceBasedRefinementArgs.Companion.fromDoc(doc: Document) =
			TomographyReferenceBasedRefinementArgs(
	...
	override suspend fun data() =
		TomographyReferenceBasedRefinementData(
			commonData(),
			args,
			diagramImageURL(),
			jobInfoString ?: ""
