...
class DrgnMap(doc: Document) {

    companion object {

        fun get(jobId: String, drgnMapId: String): DrgnMap? =
            Database.instance.drgnMaps.get(jobId, drgnMapId)
                ?.let { DrgnMap(it) }

        fun <R> getAll(jobId: String, block: (Sequence<DrgnMap>) -> R): R =
            Database.instance.drgnMaps.getAll(jobId) {
                block(it.map { innerIt -> DrgnMap(innerIt) })
            }
	...
	val jobId: String = doc.getString("jobId") ?: throw NoSuchElementException("drgnMap document has no job id")
	val drgnMapId: String = doc.getString("drgnMapId") ?: throw NoSuchElementException("drgnMap document has no drgnMap id")
	val timestamp: Instant = Instant.ofEpochMilli(doc.getLong("timestamp"))
    val classNum: Int = doc.getInteger("classNum") ?: throw NoSuchElementException("drgnMap document has no class number")
    val iteration: Int = doc.getInteger("iteration") ?: throw NoSuchElementException("drgnMap document has no iteration number")
...
//    )
    ...
    fun getLog(job: Job): String {
        ...
        val drgnMapString = filenameFragment(job)
        val searchLog = dir.resolve("${drgnMapString}_msearch.log")
			.takeIf { it.exists() }
			?.readString()
			?: ""
        val reconstLog = dir.resolve("${drgnMapString}_mreconst.log")
			.takeIf { it.exists() }
			?.readString()
			?: ""
        return searchLog + "\n" + reconstLog
