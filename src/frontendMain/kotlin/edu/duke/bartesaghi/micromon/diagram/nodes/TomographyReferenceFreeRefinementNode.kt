...
class TomographyReferenceFreeRefinementNode(
	viewport: Viewport,
	diagram: Diagram,
	project: ProjectData,
	job: TomographyReferenceFreeRefinementData
) : Node(viewport, diagram, type, config, project, job) {

	val job get() = baseJob as TomographyReferenceFreeRefinementData

	companion object : NodeClientInfo {

		override val config = TomographyReferenceFreeRefinementNodeConfig
		override val type = MicromonDiagrams.nodeType(config, "fas fa-cube")
		override val jobClass = TomographyReferenceFreeRefinementData::class
		override val urlFragment = "tomographyReferenceFreeRefinement"

		override fun makeNode(viewport: Viewport, diagram: Diagram, project: ProjectData, job: JobData) =
			TomographyReferenceFreeRefinementNode(viewport, diagram, project, job as TomographyReferenceFreeRefinementData)
		...
		override suspend fun showUseDataForm(viewport: Viewport, diagram: Diagram, project: ProjectData, outNode: Node, input: CommonJobData.DataId, copyFrom: Node?, callback: (Node) -> Unit) {
			val defaultArgs = (copyFrom as TomographyReferenceFreeRefinementNode?)?.job?.args
				?: JobArgs.fromNext(TomographyReferenceFreeRefinementArgs(newArgValues(project, input), null))
			form(config.name, outNode, defaultArgs, true) { args ->

				// save the node to the server
				AppScope.launch {
					val data = Services.tomographyReferenceFreeRefinement.addNode(project.owner.id, project.projectId, input, args)

					// send the node back to the diagram
					callback(TomographyReferenceFreeRefinementNode(viewport, diagram, project, data))
				}
			}
		}
		...
		override suspend fun makeJob(project: ProjectData, argValues: ArgValuesToml, input: CommonJobData.DataId?): JobData {
			...
			val args = TomographyReferenceFreeRefinementArgs(
				values = argValues,
				filter = null
			)
			return Services.tomographyReferenceFreeRefinement.addNode(project.owner.id, project.projectId, input, args)
		}

		override suspend fun getJob(jobId: String): TomographyReferenceFreeRefinementData =
			Services.tomographyReferenceFreeRefinement.get(jobId)

		override val pypArgs = ServerVal {
			Args.fromJson(Services.tomographyReferenceFreeRefinement.getArgs())
		}

		private fun form(caption: String, upstreamNode: Node, args: JobArgs<TomographyReferenceFreeRefinementArgs>?, enabled: Boolean, onDone: (TomographyReferenceFreeRefinementArgs) -> Unit) = AppScope.launch {
			...
			val form = win.formPanel<TomographyReferenceFreeRefinementArgs>().apply {
				...
				add(TomographyReferenceFreeRefinementArgs::filter,
				...
				add(TomographyReferenceFreeRefinementArgs::values, ArgsForm(pypArgs, listOf(upstreamNode), enabled, config.configId))
			}

			// use the none filter option for the particles name in the form,
			// since the control can't handle nulls
			val mapper = ArgsMapper<TomographyReferenceFreeRefinementArgs>(
				toForm = { args ->
					var a = args
					if (a.filter == null) {
	...
	override fun onEdit(enabled: Boolean) {
		form(job.numberedName, getUpstreamNodeOrThrow(), job.args, enabled) { newArgs ->
			...
			if (diff.shouldSave) {
				AppScope.launch {
					baseJob = Services.tomographyReferenceFreeRefinement.edit(baseJob.jobId, diff.newNextArgs(newArgs))
					edited()
				}
			}
		}
