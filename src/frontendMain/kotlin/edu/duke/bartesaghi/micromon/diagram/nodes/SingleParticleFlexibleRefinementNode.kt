...
class SingleParticleFlexibleRefinementNode(
	viewport: Viewport,
	diagram: Diagram,
	project: ProjectData,
	job: SingleParticleFlexibleRefinementData
) : Node(viewport, diagram, type, config, project, job) {

	val job get() = baseJob as SingleParticleFlexibleRefinementData
	...
	companion object : NodeClientInfo {
		...
		override val jobClass = SingleParticleFlexibleRefinementData::class
		override val urlFragment = "singleParticleFlexibleRefinement"
		...
		override fun showUseDataForm(viewport: Viewport, diagram: Diagram, project: ProjectData, outNode: Node, input: CommonJobData.DataId, copyFrom: Node?, andCopyData: Boolean, callback: (Node) -> Unit) {
			...
			form(config.name, outNode, defaultArgs, true) { args ->
				...
				AppScope.launch {
					val data = Services.singleParticleFlexibleRefinement.addNode(project.owner.id, project.projectId, input, args)

					// send the node back to the diagram
					callback(SingleParticleFlexibleRefinementNode(viewport, diagram, project, data))
				}
			}
		}
		...
		override suspend fun makeJob(project: ProjectData, argValues: ArgValuesToml, input: CommonJobData.DataId?): JobData {
			...
			val args = SingleParticleFlexibleRefinementArgs(
				values = argValues
			)
			return Services.singleParticleFlexibleRefinement.addNode(project.owner.id, project.projectId, input, args)
		}

		override suspend fun getJob(jobId: String): SingleParticleFlexibleRefinementData =
			Services.singleParticleFlexibleRefinement.get(jobId)

		override val pypArgs = ServerVal {
			Args.fromJson(Services.singleParticleFlexibleRefinement.getArgs())
		}

		private fun form(caption: String, upstreamNode: Node, args: JobArgs<SingleParticleFlexibleRefinementArgs>?, enabled: Boolean, onDone: (SingleParticleFlexibleRefinementArgs) -> Unit) = AppScope.launch {
			...
			val form = win.formPanel<SingleParticleFlexibleRefinementArgs> {
				add(SingleParticleFlexibleRefinementArgs::values, ArgsForm(pypArgs, listOf(upstreamNode), enabled, config.configId))
			}

			// by default, copy args values from the upstream node
			val argsOrCopy: JobArgs<SingleParticleFlexibleRefinementArgs> = args
				?: JobArgs.fromNext(SingleParticleFlexibleRefinementArgs(
	...
	override fun onEdit(enabled: Boolean) {
		form(job.numberedName, getUpstreamNodeOrThrow(), job.args, enabled) { newArgs ->
			...
			if (diff.shouldSave) {
				AppScope.launch {
					baseJob = Services.singleParticleFlexibleRefinement.edit(baseJob.jobId, diff.newNextArgs(newArgs))
					edited()
				}
			}
		}
