...
import edu.duke.bartesaghi.micromon.views.TomographyMiloTrainView
	...
	companion object : NodeClientInfo {
		...
		override suspend fun showUseDataForm(viewport: Viewport, diagram: Diagram, project: ProjectData, outNode: Node, input: CommonJobData.DataId, copyFrom: Node?, callback: (Node) -> Unit) {
			val defaultArgs = (copyFrom as TomographyMiloTrainNode?)?.job?.args
				?: JobArgs.fromNext(TomographyMiloTrainArgs(newArgValues(project, input), null))
	...
	override fun renderContent(refreshImages: Boolean) {
		...
		content {
			button(className = "image-button", onClick = {
				TomographyMiloTrainView.go(viewport, project, job)
			}) {
