...
fun Widget.onGoToSingleParticleSessionData(viewport: Viewport, project: ProjectData, job: SingleParticleSessionDataData) {
	onShow(SingleParticleSessionDataView.path(project, job)) {
		viewport.setView(SingleParticleSessionDataView(project, job))
	}
}

class SingleParticleSessionDataView(val project: ProjectData, val job: SingleParticleSessionDataData) : View {

	companion object : Routed {

		override fun register(routing: Routing, viewport: Viewport) {
			routing.registerParams("^/project/($urlToken)/($urlToken)/singleParticleSessionData/($urlToken)$") { userId, projectId, jobId ->
				AppScope.launch {
					try {
						val project = Services.projects.get(userId, projectId)
						val job = Services.singleParticleSessionData.get(jobId)
						viewport.setView(SingleParticleSessionDataView(project, job))
					} catch (t: Throwable) {
		...
		fun path(project: ProjectData, job: SingleParticleSessionDataData) = "/project/${project.owner.id}/${project.projectId}/singleParticleSessionData/${job.jobId}"

		fun go(viewport: Viewport, project: ProjectData, job: SingleParticleSessionDataData) {
	...
	override fun init(viewport: Viewport) {
		...
		viewport.navbarElem.apply {
			navex {
				...
				navLink(job.numberedName, icon = SingleParticleSessionDataNode.type.iconClass)
					.onGoToSingleParticleSessionData(viewport, project, job)
			}
		}
		...
		AppScope.launch {
			...
			tabs = elem.lazyTabPanel {
				...
				persistence = Storage::singleParticleSessionDataTabIndex
			...
			this@SingleParticleSessionDataView.connector = connector
