...
fun Widget.onGoToTomographyImportData(viewport: Viewport, project: ProjectData, job: TomographyImportDataData) {
	onShow(TomographyImportDataView.path(project, job)) {
		viewport.setView(TomographyImportDataView(project, job))
	}
}

class TomographyImportDataView(val project: ProjectData, val job: TomographyImportDataData) : View {

	companion object : Routed {

		override fun register(routing: Routing, viewport: Viewport) {
			routing.registerParams("^/project/($urlToken)/($urlToken)/tomographyImportData/($urlToken)$") { userId, projectId, jobId ->
				AppScope.launch {
					try {
						val project = Services.projects.get(userId, projectId)
						val job = Services.tomographyImportData.get(jobId)
						viewport.setView(TomographyImportDataView(project, job))
					} catch (t: Throwable) {
	...
	override fun init(viewport: Viewport) {
		...
		viewport.navbarElem.apply {
			navex {
				...
				navLink(job.numberedName, icon = TomographyImportDataNode.type.iconClass)
					.onGoToTomographyImportData(viewport, project, job)
			}
		}
		...
		AppScope.launch {
			...
			tabs = elem.lazyTabPanel {
				...
				persistence = Storage::tomographyImportDataTabIndex
			...
			this@TomographyImportDataView.connector = connector
