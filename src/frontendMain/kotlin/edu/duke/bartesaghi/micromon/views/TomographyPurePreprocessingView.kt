...
fun Widget.onGoToTomographyPurePreprocessing(viewport: Viewport, project: ProjectData, job: TomographyPurePreprocessingData) {
	onShow(TomographyPurePreprocessingView.path(project, job)) {
		viewport.setView(TomographyPurePreprocessingView(project, job))
	}
}

class TomographyPurePreprocessingView(val project: ProjectData, val job: TomographyPurePreprocessingData) : View {

	companion object : Routed {

		override fun register(routing: Routing, viewport: Viewport) {
			routing.registerParams("^/project/($urlToken)/($urlToken)/tomographyPurePreprocessing/($urlToken)$") { userId, projectId, jobId ->
				AppScope.launch {
					try {
						val project = Services.projects.get(userId, projectId)
						val job = Services.tomographyPurePreprocessing.get(jobId)
						viewport.setView(TomographyPurePreprocessingView(project, job))
					} catch (t: Throwable) {
		...
		fun path(project: ProjectData, job: TomographyPurePreprocessingData) = "/project/${project.owner.id}/${project.projectId}/tomographyPurePreprocessing/${job.jobId}"

		fun go(viewport: Viewport, project: ProjectData, job: TomographyPurePreprocessingData) {
	...
	override fun init(viewport: Viewport) {
		...
		viewport.navbarElem.apply {
			navex {
				...
				navLink(job.numberedName, icon = TomographyPurePreprocessingNode.type.iconClass)
					.onGoToTomographyPurePreprocessing(viewport, project, job)
			}
		}
		...
		AppScope.launch {
			...
			tabs = elem.lazyTabPanel {
				...
				persistence = Storage::tomographyPurePreprocessingTabIndex
			...
			this@TomographyPurePreprocessingView.connector = connector
