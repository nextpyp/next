...
fun Widget.onGoToTomographySegmentationClosed(viewport: Viewport, project: ProjectData, job: TomographySegmentationClosedData) {
	onShow(TomographySegmentationClosedView.path(project, job)) {
		viewport.setView(TomographySegmentationClosedView(project, job))
	}
}

class TomographySegmentationClosedView(val project: ProjectData, val job: TomographySegmentationClosedData) : View {

	companion object : Routed {

		override fun register(routing: Routing, viewport: Viewport) {
			routing.registerParams("^/project/($urlToken)/($urlToken)/tomographySegmentationClosed/($urlToken)$") { userId, projectId, jobId ->
				AppScope.launch {
					try {
						val project = Services.projects.get(userId, projectId)
						val job = Services.tomographySegmentationClosed.get(jobId)
						viewport.setView(TomographySegmentationClosedView(project, job))
					} catch (t: Throwable) {
		...
		fun path(project: ProjectData, job: TomographySegmentationClosedData) = "/project/${project.owner.id}/${project.projectId}/tomographySegmentationClosed/${job.jobId}"

		fun go(viewport: Viewport, project: ProjectData, job: TomographySegmentationClosedData) {
	...
	override fun init(viewport: Viewport) {
		...
		viewport.navbarElem.apply {
			navex {
				...
				navLink(job.numberedName, icon = TomographySegmentationClosedNode.type.iconClass)
					.onGoToTomographySegmentationClosed(viewport, project, job)
			}
		}
		...
		AppScope.launch {
			...
			this@TomographySegmentationClosedView.connector = connector
