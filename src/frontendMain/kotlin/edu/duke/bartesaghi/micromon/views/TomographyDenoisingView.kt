...
import edu.duke.bartesaghi.micromon.diagram.nodes.TomographyDenoisingNode
...
fun Widget.onGoToTomographyDenoising(viewport: Viewport, project: ProjectData, job: TomographyDenoisingData) {
	onShow(TomographyDenoisingView.path(project, job)) {
		viewport.setView(TomographyDenoisingView(project, job))
	}
}

class TomographyDenoisingView(val project: ProjectData, val job: TomographyDenoisingData) : View {

	companion object : Routed {

		override fun register(routing: Routing, viewport: Viewport) {
			routing.registerParams("^/project/($urlToken)/($urlToken)/tomographyDenoising/($urlToken)$") { userId, projectId, jobId ->
				AppScope.launch {
					try {
						val project = Services.projects.get(userId, projectId)
						val job = Services.tomographyDenoising.get(jobId)
						viewport.setView(TomographyDenoisingView(project, job))
					} catch (t: Throwable) {
		...
		fun path(project: ProjectData, job: TomographyDenoisingData) = "/project/${project.owner.id}/${project.projectId}/tomographyDenoising/${job.jobId}"

		fun go(viewport: Viewport, project: ProjectData, job: TomographyDenoisingData) {
	...
	override fun init(viewport: Viewport) {
		...
		viewport.navbarElem.apply {
			navex {
				...
				navLink(job.numberedName, icon = TomographyDenoisingNode.type.iconClass)
					.onGoToTomographyDenoising(viewport, project, job)
			}
		}
		...
		AppScope.launch {
			...
			val connector = WebsocketConnector(RealTimeServices.tomographyDenoising) { signaler, input, output ->
			...
			this@TomographyDenoisingView.connector = connector
